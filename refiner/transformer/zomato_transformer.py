...
from refiner.models.unrefined import ZomatoData
...
class ZomatoTransformer(DataTransformer):
    ...
    def transform(self, data: Dict[str, Any]) -> List[Base]:
        ...
        # Parse the claimed date using the utility function
        claimed_date = parse_timestamp(zomato_data.claimedDate)
        
        # Create Zomato account instance
        account = ZomatoAccount(
            data_type=zomato_data.type,
            claimed_date=claimed_date,
            witnesses=zomato_data.witnesses,
            wallet_address=zomato_data.walletAddress,
        ...
        # Create order instances for each order
